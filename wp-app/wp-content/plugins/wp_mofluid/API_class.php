...

{
	...
	*/
	...
	function getCategoriesList($flag = NULL)
	{
		...
		foreach($all_categories as $key => $value) {
			...
			$child_nodes = get_terms("product_cat", $child_args);
			if (is_array($child_nodes) && count($child_nodes) > 0) {
				$child_arr = array();
				$count_child = 0;
				foreach($child_nodes as $child_nodes_key => $child_nodes_val) {
					$child_arr[$count_child]['id'] = $child_nodes_val->term_id;
					$child_arr[$count_child]['name'] = $child_nodes_val->name;
					$count_child++;
				}

				$cat_arr[$count]["children"] = $child_arr;
			}
	...
	function get_product_categories($flag = NULL)
	{
		$taxonomy = 'product_cat';
		$orderby = 'name';
		$show_count = 0; // 1 for yes, 0 for no
	...
	function get_product_subcategory($cat_id)
	{
		$taxonomy = 'product_cat';
		$orderby = 'name';
		$show_count = 0; // 1 for yes, 0 for no
	...
	function get_product_byID($cat_id)
	{
		global $wpdb;
		$args = array(
			'post_type' => 'product',
		...
		if (isset($_GET['sorttype']) && $_GET['sorttype'] == 'price' && isset($_GET['sortorder']) && $_GET['sortorder'] == 'desc') {
			$args["meta_key"] = "_price";
			$args["orderby"] = "meta_value_num";
			$args["order"] = "DESC";
		}
		else
		if (isset($_GET['sorttype']) && $_GET['sorttype'] == 'name' && isset($_GET['sortorder']) && $_GET['sortorder'] == 'asc') {
			$args["orderby"] = "post_title";
			$args["order"] = "ASC";
		}
		else
		if (isset($_GET['sorttype']) && $_GET['sorttype'] == 'name' && isset($_GET['sortorder']) && $_GET['sortorder'] == 'desc') {
			$args["orderby"] = "post_title";
			$args["order"] = "DESC";
		}
		else
		if (isset($_GET['sorttype']) && $_GET['sorttype'] == 'entity_id' && isset($_GET['sortorder']) && $_GET['sortorder'] == 'asc') {
			$args["orderby"] = "ID";
			$args["order"] = "ASC";
		}
		else
		if (isset($_GET['sorttype']) && $_GET['sorttype'] == 'entity_id' && isset($_GET['sortorder']) && $_GET['sortorder'] == 'desc') {
			$args["orderby"] = "ID";
			$args["order"] = "DESC";
		}
	...
	/*
	* Get 10 featured products
	* @param catid
	* @return Array
	*/
	public

	function getFeaturedProducts($showproducts = 10)
	{
		global $wpdb;
		...
		if ($loop->have_posts()) {
			while ($loop->have_posts()):
				...
				if ($product->product_type == "simple") {
					$product_arr[$count]['id'] = $product->id;
					$product_arr[$count]['name'] = $product->get_title();
					$product_arr[$count]['image'] = wp_get_attachment_url(get_post_thumbnail_id($product->id));
					$product_arr[$count]['sku'] = $product->get_sku();
					$product_arr[$count]['price'] = $product->get_regular_price();
					$product_arr[$count]['special_price'] = strpos($product->get_sale_price() , '.') !== false ? $product->get_sale_price() : $product->get_sale_price() . '.00';
					$product_arr[$count]['created_date'] = $product->post->post_date;
					$product_arr[$count]['currency_symbol'] = $currency_symbol;
					if ($product->is_in_stock()) {
						$product_arr[$count]['is_stock_status'] = 1;
					}
					else {
						$product_arr[$count]['is_stock_status'] = 0;
					}
			...
			endwhile;
			$json_main['products_list'] = $product_arr;
			echo $_GET["callback"] . json_encode($json_main);
		}
	}
	...
	*/
	...
	function getNewProducts($showproducts = 10)
	{
		global $wpdb;
		$args = array(
			'post_type' => 'product',
		...
		if ($loop->have_posts()) {
			...
			// ~ echo "<pre>"; print_r($json_main['products_list']); die;
	...
	function payment_methods()
	{
		...
		foreach($result as $key => $value) {
			...
			$json_r['payment_method_order_code'] = $value['payment_method_order_code']; //$value->id;
			...
			$json_r['payment_method_account_key'] = $value['payment_method_account_key'];
	...
	function create_user($args)
	{
		...
		if (count($user_id->errors) > 0) {
			...
			// get_userdata($user_id)->data->user_status;
	...
	function get_myprofile($customerid)
	{
		$json_cus = array();
		...
		if ($userinfo) {
			...
			// echo '<pre>',print_r($cus_data),'</pre>'; die;
			// var_dump($cus_data);die;
	...
	function productdetail($p_id)
	{
		ini_set('display_errors', 1);
		ini_set('display_startup_errors', 1);
		error_reporting(E_ALL);
		$p_id = str_replace(array(
			"stock_status0",
			"stock_status1"
		) , '', $p_id);
		$json_r = array();
		...
		if (!empty($product)) {
			...
			if ($product->is_on_sale()) $json_r['status'] = 1;
			else $json_r['status'] = 0;
			$json_r['category'] = $p_cat;
			$json_r['general'] = array(
				'name' => $product->get_title() ,
				'sku' => $product->get_sku() ,
				'weight' => $product->get_weight()
			);
			$json_r['price'] = array(
				'regular' => ($product->regular_price ? $product->regular_price : 0.00) ,
				'final' => ($product->sale_price ? $product->sale_price : $product->regular_price)
			);
			$count = 0;
			...
			$json_r['reviews'] = array(
				'total' => ''
			);
			$json_r['description'] = array(
				'short' => base64_encode($product->post->post_excerpt) ,
				'full' => base64_encode(preg_replace("/<img[^>]+\>/i", "", $product->post->post_content))
			);
	...
	function productdetaildescription($p_id)
	{
		$p_id = str_replace(array(
			"stock_status0",
			"stock_status1"
		...
		if (!empty($product)) {
			...
			if ($p_category) {
				...
				$json_r['price'] = $product->regular_price;
				$json_r['sprice'] = $product->sale_price;
				$count = 0;
				$json_r['image'][$count] = wp_get_attachment_url(get_post_thumbnail_id($product->id));
				$gal_image_id = $product->get_gallery_attachment_ids();
				foreach($gal_image_id as $key => $val) {
				...
				// ~ $json_r['description'] = array('shortdes' => base64_encode($product->post->post_excerpt), 'full' => base64_encode($product->post->post_content));

				$json_r['description'] = $product->post->post_content;
				$json_r['shortdes'] = $product->post->post_excerpt;
				$json_r['currencysymbol'] = get_woocommerce_currency_symbol($currency);
				$json_r['discount'] = '0';
				$json_r['tax'] = '0';
				$json_r['tax_type'] = '0';
				$json_r['url'] = $product->get_permalink();
				...
				if ($stock_enable == 'yes') {
					if (get_post_meta($product->id, '_manage_stock', TRUE) == 'yes' && get_post_meta($product->id, '_stock', TRUE) > 0) {
						...
						$json_r['quantity'] = intval(get_post_meta($product->id, '_stock', TRUE));
						$json_r['stock']['min_qty'] = intval(get_post_meta($product->id, '_stock', TRUE));

						// ~ $json_r['stock']['is_in_stock'] = 1;

						$json_r['is_in_stock'] = 1;
					}
					else {
						if (get_post_meta($product->id, '_stock_status', TRUE) == 'instock') {
							$json_r['stock']['qty'] = 99;
	...
	function productdetailimage($p_id)
	{
		$p_id = str_replace(array(
			"stock_status0",
			"stock_status1"
		...
		if (empty($json_r['image'][$count])) $status = 0;
		else $status = 1;
		$json_r['status'] = $status;
		echo $_GET["callback"] . json_encode($json_r);

		// ~ echo '<pre>',print_r($json_r),'</pre>';die;
	...
	*/
	...
	function getStoreInfo()
	{
		...
		$store_arr['cache_setting'] = array(
			'cache_time' => '30',
			'status' => '1'
		);
		$store_arr["logo"] = $theme_info->logo;
		$store_arr["banner"] = $theme_info->banner;
		$store = array(
			'store' => $store_arr
		);
		/*         * ********************************************************************************************* */
		...
		$url_arr = array();
		$url_arr['root'] = get_bloginfo('url');
		$url_arr['store'] = get_bloginfo('url');
		$url_final_arr = array(
			'url' => $url_arr
		);
		...
		// $currency_name = get_woocommerce_currency_name();
		...
		$theme_res["logo"] = array(
			"image" => array(
				array(
					"mofluid_image_value" => $theme_info->logo
				) ,
			) ,
			"alt" => "Mufluid Commerce"
		);
		$theme_res["banner"] = array(
			"image" => array(
				array(
		...
		$theme_res_final = array(
			"theme" => $theme_res
		);

		// Merge All Arrays:

		$out_array = array_merge($store, $timezone, $url_final_arr, $currency_final, $theme_res_final);
	...
	function stateList($args)
	{
		...
		$c = new WC_Countries();
		$statelist = $c->get_states($country);
		if ($statelist) {
			$count = 0;
			$json_r = array();
			foreach($statelist as $countryInfo) {

				// $json_r[$count]['region_id'] = $key;
				// $json_r[$count++]['region_name'] = $value;
	...
	function productQuantity($product_ids)
	{
		$json_r = array();
		...
		foreach($p_id as $key => $value) {
			$product = get_product($value);
			if (!empty($product)) $json_r[$value] = $product->get_stock_quantity();

			// var_dump($product->get_stock_quantity());

		}

		if (count($json_r) < 1) $json_r = FALSE;
		echo $_GET["callback"] . "(" . json_encode($json_r) . ");";
	...
	function createorder($args)
	{
		...
		if ($user_info || $user_info == null) {
			...
			// ~ echo '<pre>',print_r($order_data),'</pre>'; die;
			...
			if (is_wp_error($order_id)) {
				...
				if (strtoupper($paymentmethod) == 'cashondelivery' || strtoupper($paymentmethod) == 'free' || $paymentmethod == 'FREE') {
					$orderObj->update_status('processing');
				}

				$order->imported = true;
				$paymentMethodTitle = strtolower($paymentmethod) == "paypal" ? "PayPal" : strtoupper($paymentmethod);

				// add a bunch of meta data

				add_post_meta($order_id, 'transaction_id', $transactionid, true);
				add_post_meta($order_id, '_payment_method_title', $paymentMethodTitle, true);
				...
				add_post_meta($order_id, '_order_key', 'wc_' . apply_filters('woocommerce_generate_order_key', uniqid('order_')) , true);
				add_post_meta($order_id, '_order_currency', get_woocommerce_currency() , true);
				...
				if (!empty($couponCode)) {
					...
					if ($couponData['status'] == 1 && $couponData['is_active'] == 1 && $couponData['simple_action'] == 'fixed_cart') {
						$newOrderTotal = $total - $couponData['discount_amount'];
						if ($newOrderTotal < 0) {
							$newOrderTotal = '0.00';
						}

						update_post_meta($order_id, '_order_total', $newOrderTotal, $total);
					...
					if ($couponData['status'] == 1 && $couponData['is_active'] == 1 && $couponData['simple_action'] == 'percent') {
						$newOrderTotal = $total - $total * $couponData['discount_amount'] / 100;
						if ($newOrderTotal < 0) {
							$newOrderTotal = '0.00';
						}
		...
		if ($order_id && strtoupper($paymentmethod) == 'cashondelivery') {
			$sendmail_args = array(
				'orderid' => $order_id
			);
	...
	function setaddress($args)
	{
		...
		if ($u) {
			...
			update_user_meta($cst_id, 'shipping_first_name', isset($shippAdd->firstname) ? stripcslashes($shippAdd->firstname) : stripslashes($billAdd->firstname));
			update_user_meta($cst_id, 'shipping_last_name', isset($shippAdd->lastname) ? stripcslashes($shippAdd->lastname) : stripslashes($billAdd->lastname));
			update_user_meta($cst_id, 'shipping_address_1', isset($shippAdd->street) ? stripcslashes($shippAdd->street) : stripslashes($billAdd->street));
			update_user_meta($cst_id, 'shipping_city', isset($shippAdd->city) ? stripcslashes($shippAdd->city) : stripslashes($billAdd->city));
			update_user_meta($cst_id, 'shipping_postcode', isset($shippAdd->postcode) ? stripcslashes($shippAdd->postcode) : stripslashes($billAdd->postcode));
			update_user_meta($cst_id, 'shipping_state', isset($shippAdd->region) ? stripcslashes($shippAdd->region) : stripslashes($billAdd->region));
			update_user_meta($cst_id, 'shipping_country', isset($shippAdd->country) ? stripcslashes($shippAdd->country) : stripslashes($billAdd->country));
			update_user_meta($cst_id, 'shipping_phone', isset($shippAdd->phone) ? stripcslashes($shippAdd->phone) : stripslashes($billAdd->phone));
			update_user_meta($cst_id, 'shipping_email', isset($shippAdd->email) ? stripcslashes($shippAdd->email) : stripslashes($billAdd->email));
	...
	function validate_currency($args)
	{
		$currency = $args['currency'];
	...
	function listShipping()
	{
		...
		if ($json_r == NULL) $json_r[] = 'Shipping method not found';
		echo $_GET["callback"] . json_encode($json_r);
	...
	function couponDetails($args, $rsponseArr = false)
	{
		$coupon_code = $args['couponCode'];
		...
		if ($coupon->id != null) {
			$p_status = 0;
			if (get_post_status($coupon->id) == 'publish') $p_status = 1;
		...
		if ($rsponseArr == false) echo $_GET["callback"] . json_encode($json_r);
		else return $json_r;
...
<table cellspacing="0" cellpadding="6" style="width:100%;border:1px solid #eee" border="1"><thead><tr><th scope="col" style="text-align:left;border:1px solid #eee">Product</th>
			...
			<th scope="col" style="text-align:left;border:1px solid #eee">Price</th>
					...
					foreach($order_info["products"] as $key => $val) {
						$content.= '<tr><td style="text-align:left;vertical-align:middle;border:1px solid #eee;word-wrap:break-word">' . $val["name"] . '<br /><small></small></td>
...
<table cellspacing="0" cellpadding="0" style="width:100%;vertical-align:top" border="0"><tbody><tr><td valign="top" width="50%">
	...
	function getShippingDetail($args)
	{
	...
	function countryStateList($args)
	{
		$country = $args['country'];
		$json_r = array();
	...
	function myorders($arg)
	{
		...
		if (count($customer_orders)) {
			...
			foreach($customer_orders as $order) {
				$status = $order->post_status; //=='trash'
				if ($status == 'trash') {
				}
				else {
					$json_main['id'] = $order->ID;
					$json_main['order_id'] = " " . $order->ID;
					$json_main['status'] = $order->post_status;
					
					$json_main['order_date'] = $order->post_date;
					$json_main['couponUsed'] = get_post_meta($order->ID, 'couponUsed', TRUE);
					$json_main['couponCode'] = get_post_meta($order->ID, 'couponCode', TRUE);
					$json_main['discount_amount'] = get_post_meta($order->ID, '_order_discount', TRUE);
					$json_main['grand_total'] = get_post_meta($order->ID, '_order_total', TRUE);
					$json_cus = array();
					...
					$orderData = wc_get_order( $order->ID );
					foreach($orderData->get_items() as $item_id => $item) {
						$p_id = version_compare( WC_VERSION, '3.0', '<' ) ? $item['product_id'] : $item->get_product_id();
						$product = get_product($p_id);
						if (!empty($product)) {
							$p_category = get_the_terms($product->id, 'product_cat');
							//if ($p_category) {
...
// 								}
								...
								$json_c['total_item_count'][$i] = $product->get_price() * $item['qty']; //$item['item_meta'][_line_total][0];
					...
					$json_main['tax_amount'] = $order->tax_display_cart;
					...
					$json_main['payment_method']['payment_method_title'] = get_post_meta($order->id, '_payment_method_title', TRUE);
					$json_main['payment_method']['payment_method_code'] = get_post_meta($order->id, '_payment_method', TRUE);
					foreach($orderData->get_items('shipping') as $ship) {
					...
					$json_main['shipping_amount'] = $orderData->get_total_shipping();
	...
	function retrieve_password($user_login)
	{
		...
		if (!$allow) return new WP_Error('no_password_reset', __('Password reset is not allowed for this user'));
		else
		...
		*/
		...
		if ($message && !wp_mail($user_email, wp_specialchars_decode($title) , $message)) wp_die(__('The e-mail could not be sent.') . "<br />\n" . __('Possible reason: your host may have disabled the mail() function.'));
		return true;
	}

	public

	function preparequote($args)
	{
		global $woocommerce;
		...
		// ~ echo '<pre>',print_r($shippAdd),'</pre>'; die;

		if ($u) {
			update_user_meta($cst_id, 'billing_first_name', stripslashes($billAdd->firstName));
			update_user_meta($cst_id, 'billing_last_name', stripslashes($billAdd->lastName));
			update_user_meta($cst_id, 'billing_address_1', stripslashes($billAdd->address));
			update_user_meta($cst_id, 'billing_city', stripslashes($billAdd->city));
			update_user_meta($cst_id, 'billing_postcode', stripslashes($billAdd->zipcode));
			update_user_meta($cst_id, 'billing_state', stripslashes($billAdd->state->region_name));
			update_user_meta($cst_id, 'billing_country', stripslashes($billAdd->country));
			update_user_meta($cst_id, 'billing_phone', stripslashes($billAdd->contactNo));
			update_user_meta($cst_id, 'billing_email', stripslashes($billAdd->email));
			update_user_meta($cst_id, 'shipping_first_name', isset($shippAdd->firstName) ? stripcslashes($shippAdd->firstName) : stripslashes($billAdd->firstname));
			update_user_meta($cst_id, 'shipping_last_name', isset($shippAdd->lastName) ? stripcslashes($shippAdd->lastName) : stripslashes($billAdd->lastName));
			update_user_meta($cst_id, 'shipping_address_1', isset($shippAdd->address) ? stripcslashes($shippAdd->address) : stripslashes($billAdd->street));
			update_user_meta($cst_id, 'shipping_city', isset($shippAdd->city) ? stripcslashes($shippAdd->city) : stripslashes($billAdd->city));
			update_user_meta($cst_id, 'shipping_postcode', isset($shippAdd->zipcode) ? stripcslashes($shippAdd->zipcode) : stripslashes($billAdd->postcode));
			update_user_meta($cst_id, 'shipping_state', isset($shippAdd->state->region_name) ? stripcslashes($shippAdd->state->region_name) : stripslashes($billAdd->state->region_name));
			update_user_meta($cst_id, 'shipping_country', isset($shippAdd->country) ? stripcslashes($shippAdd->country) : stripslashes($billAdd->country));
			update_user_meta($cst_id, 'shipping_phone', isset($shippAdd->contactNo) ? stripcslashes($shippAdd->contactNo) : stripslashes($billAdd->contactNo));
			update_user_meta($cst_id, 'shipping_email', isset($shippAdd->email) ? stripcslashes($shippAdd->email) : stripslashes($billAdd->email));
		}
		...
		foreach($shipping_methods as $id => $shipping_method) {
			if (isset($shipping_method->enabled) && $shipping_method->enabled == 'yes') {
				$check = $shipping_method->id;
				if ($check == "flat_rate") $code = "flatrate_flatrate";
				else
				if ($check == "free_shipping") $code = "freeshipping_freeshipping";
				else $code = $shipping_method->id;
				//echo "<pre>";
				...
				$active_methods[] = array(
					...
					'carrier_title' => $shipping_method->method_title,
					'carrier' => $shipping_method->title,
					'price' => $shipping_method->fee?$shipping_method->fee:0,
					'cost' => $shipping_method->cost?$shipping_method->cost:0,
					'code' => $code,
					"error_message" => "",
					"address_id" => "",
					"created_at" => "",
		...
		$res["shipping_method"] = $active_methods;
		/*
		$shipping = new WC_Shipping();
		$ships = $shipping->load_shipping_methods();
		...
		// ~ $shipping_dropdown_option .= "<option id='$shiping_method_id' value= '".($shiping_method_id=='free_shipping'?"Free Shipping":$shiping_method_id)."' 'free_shipping' price = '$fee' description='$title'>$title</option>";

		$shipping_dropdown_option .= $shiping_method_id=='free_shipping'?"Free Shipping":$shiping_method_id;
		}
		}
		...
		*/
		...
		// ~ echo '<pre>',print_r($shipping_dropdown_option),'</pre>'; die;
		...
		// ~ echo '<pre>',print_r($couponData),'</pre>';
		...
		if (isset($res["total_amount"]) && $res["total_amount"] < 0) {
	...
	function get_orderinfo($args, $flag = null)
	{
		...
		if ($order->post != null) {
			...
			$json['currency'] = array(
				"code" => $order->order_currency,
				"symbol" => get_woocommerce_currency_symbol($order->order_currency) ,
				"current" => $order->order_currency
			);
			...
			$json['shipping'] = array(
				...
				"amount" => method_exists($order, get_shipping) ? $order->get_shipping() : "0.00"
			);
		}

		if ($flag == '1') return $json;
		else echo $_GET["callback"] . json_encode($json);
	...
	function send_userregistration_mail($user_info)
	{
	...
	function ws_loginwithsocial($args)
	{
		...
		if (count($user_id->errors) > 0) {
			...
			if ($customer) {
				...
				// $user_info['lastname'] = $cus_data['last_name'][0];
	...
	function update_myprofile($user_details)
	{
		$cust_id = $user_details['customerid'];
		$bill_addr = base64_decode($user_details['billaddress']);
		$ship_addr = base64_decode($user_details['shippaddress']);

		// {"billaddress":1,"shippaddress":0,"userprofile":0}
		...
		$profile = base64_decode($user_details['profile']);
		$ship_bill_choice = $user_details['shipbillchoice'];
		$obj = json_decode($bill_addr);
		$obj1 = json_decode($ship_addr);
		$obj2 = json_decode($profile);

		// print_r($obj1);
		// ~ print_r($obj2->email); die;

		if ($ship_bill_choice == 'billingaddress') {
			$bill_fname = $obj->billfname;
			$bill_lname = $obj->billlname;
			$bill_country = $obj->billcountry;
			$bill_city = $obj->billcity;
			$bill_state = $obj->billstate->region_name;
			$bill_postcode = $obj->billpostcode;
			$bill_street1 = $obj->billstreet1;
			$bill_phone = $obj->billphone;
			$bill_email = $obj2->email;
			$ship_fname = $obj->billfname;
			$ship_lname = $obj->billlname;
			$ship_country = $obj->billcountry;
			$ship_city = $obj->billcity;
			$ship_state = $obj->billstate->region_name;
			$ship_postcode = $obj->billpostcode;
			$ship_street1 = $obj->billstreet1;
			$ship_phone = $obj->billphone;
			$ship_email = $obj2->email;
			$metas_billing = array(
				'billing_first_name' => $bill_fname,
				'billing_last_name' => $bill_lname,
				'billing_address_1' => $bill_street1,
			...
			foreach($metas_billing as $key => $value) {
				update_user_meta($cust_id, $key, $value);
				$result['billaddress'] = 1;
			}

			$metas_shipping = array(
				'shipping_first_name' => $ship_fname,
				'shipping_last_name' => $ship_lname,
				'shipping_address_1' => $ship_street1,
				...
				'shipping_email' => $ship_email
			);
			foreach($metas_shipping as $key1 => $value1) {
				update_user_meta($cust_id, $key1, $value1);
				$result['shippaddress'] = 1;
			}

			// 	try {
			// 					if($shippAddress->update_user_meta())
			// 						$result['billaddress'] = 1;
			// 				}
			//
		...
		if ($ship_bill_choice == 'shippingaddress') {
			$ship_fname = $obj1->shippfname;
			$ship_lname = $obj1->shipplname;
			$ship_country = $obj1->shippcountry;
			$ship_city = $obj1->shippcity;
			$ship_state = $obj1->shippstate->region_name;
			$ship_postcode = $obj1->shipppostcode;
			$ship_street1 = $obj1->shippstreet1;
			$ship_phone = $obj1->shippphone;
			$ship_email = $obj2->email;
			$metas_shipping = array(
				'shipping_first_name' => $ship_fname,
				'shipping_last_name' => $ship_lname,
			...
			// 					try {
			// 					if($billAddress->update_user_meta())
			// 						$result['shippaddress'] = 1;
			// 				}

			echo $_GET["callback"] . json_encode($result);
		}
		else echo $_GET["callback"] . json_encode('Error');
	}
}
