...
<div class="profile-header">
    ...
    if( is_user_logged_in() && $logged_in_user_id != $thisuser->ID ){
        ...
        $follow_text    = $wpdb->num_rows > 0 ? __('Following', 'user-profile') : __('Follow', 'user-profile');
        $follow_class   = $wpdb->num_rows > 0 ? 'following' : '';			

        printf( "<div user_id='%d' class='follow %s'><i class='icofont icofont-contact-add'></i> %s</div>", $thisuser->ID, $follow_class, $follow_text );
    }    
    ?>
