...
class WPPP_Admin_Renderer {
	...
	public function enqueue_scripts_and_styles() {
		...
		wp_register_script( 'jquery-switchbutton', $this->wppp->plugin_url . 'common/js/jquery.switchButton.js', array ( 'jquery-effects-core' ), false, true );
		if ( $this->view === 'simple' )
	...
	public function add_help_tab() {
		...
		if ( $this->view !== 'simple' ) {
			$screen->add_help_tab( array(
				...
				'content'	=> '<p>' . sprintf( __( 'WPPP supports debugging using the %s plugin. When installed and activated, debugging adds a new panel to the Debug Bar showing information about loaded textdomains, used translation implementations and translation calls, as well as information about gettext support and other details.', 'wp-performance-pack' ), '<a href="http://wordpress.org/plugins/debug-bar/">Debug Bar</a>' ) . '</p>',
			) );
		}
	}
			...
			<table class="form-table" style="clear:none">
				<tr valign="top">
					...
					<td>
						...
						<p class="description"><?php _e( 'Enables debugging, requires <a href="http://wordpress.org/plugins/debug-bar/">Debug Bar</a> Plugin.', 'wp-performance-pack' ); ?></p>
					</td>
				</tr>
			</table>
			<hr/>
	...
	?>
		<?php if ( $label !== '' ) { ?><label for="<?php echo $opt_name . '_id'; ?>"><?php echo $label; ?></label><?php } ?>
		<input id="<?php echo $opt_name . '_id'; ?>" type="checkbox" value="true" <?php echo ( $this->wppp->options[ $opt_name ] === true ) ? 'checked="checked"' : ''; ?> class="switchButton" <?php $this->e_opt_name( $opt_name ); ?> <?php echo ( $disabled === true ) ? 'disabled="true"' : ''; ?> />
	<?php
	}
	 /*
