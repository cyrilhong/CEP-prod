	...
	<td class="item">
		...
		<table class="redemption-meta">
			...
			<tr>
				...
				<td>
					<?php if ( $order ) : ?>
						<?php /* translators: %s - order number */ printf( esc_html_e( 'Order %s', 'woocommerce-pdf-product-vouchers' ), '<a href="' . get_edit_post_link( SV_WC_Order_Compatibility::get_prop( $order, 'id' ) ) . '">' . $order->get_order_number() . '</a>' ); ?>
					<?php elseif ( $user ) : ?>
