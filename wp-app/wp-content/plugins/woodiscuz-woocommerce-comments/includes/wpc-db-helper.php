...
class WPC_DB_Helper {
    ...
    public function update_comments_types() {
        ...
        if ($comments_ids) {
            ...
            $update_query = "UPDATE `" . $this->dbprefix . "comments` SET `comment_type` = 'review' WHERE `comment_ID` IN($comment_ids) AND `comment_type` LIKE '';";
            $this->db->query($update_query);
        }
    }

    public function update_comments_types_2_2_0() {
        $update_query = "UPDATE `" . $this->dbprefix . "comments` SET `comment_type` = 'review' WHERE `comment_type` = 'woodiscuz_review';";
        $this->db->query($update_query);
    }
