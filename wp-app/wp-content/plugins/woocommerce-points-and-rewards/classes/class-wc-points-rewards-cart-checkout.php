...
class WC_Points_Rewards_Cart_Checkout {
	...
	public static function get_discount_for_redeeming_points() {
		...
		foreach ( $woocommerce->cart->get_cart() as $item_key => $item ) {
			...
			// Max should be product price. As this will be applied before tax, it will respect other coupons.
				...
				if ( method_exists( $item['data'], 'get_price_including_tax' ) ) {
					$max_discount = $item['data']->get_price_including_tax();
				} else {
					$max_discount = $item['data']->get_price();
				}
		...
		if ( get_option('woocommerce_prices_include_tax') == 'no' ) {
			...
			$discount_applied = max( 0, min( $discount_applied, $woocommerce->cart->subtotal - $woocommerce->cart->discount_total - $woocommerce->cart->discount_cart ) );
		}
