	            	...
	            	if( 0 !== data.error ){
	                	...
	                	switch(slug){
                			...
                			case 'cache-performance':
                				...
                				if( $el.length ){
                					$('.sidebar-track-load-time-wrap').html(data.track_load_time_settings_content);
	                				$el.html( data.plugin_settings_content );
	                				setTimeout(function(){
	                					Optimisationio_PluginSettingsTabs.bind_events($el.find('.addon-settings'), slug);
...
var Optimisationio_Dashboard_Cache_Performance = (function($){
	...
	var $toogle_el = {};
	var $btn_el = {};
	var while_changing_load_track = false;
	...
	function on_change_track_load_time(ev){
		
		ev.preventDefault();
		ev.stopPropagation();

		if( while_changing_load_track ){ return; }

		while_changing_load_track = true;

		var $this = $toogle_el.track_load_time;
		var $grand_parent = $this.parent().parent();
		var error_msg = "An erro occured on saving value of setting 'Track my page load time'.";

		$grand_parent.addClass("is-executing");

		$.ajax({
            type: 'post',
            url: ajaxurl,
            data:{
            	action: 'optimisationio_track_page_load_time',
            	nonce: $('#optimisationio_track_load_time_nonce').val(),
            	checked: $toogle_el.track_load_time.is(":checked") ? 1 : 0
            },
            dataType: 'json',
            success: function (data, textStatus, XMLHttpRequest) {},
            error: function (data, textStatus, XMLHttpRequest) { console.error(data); }
        }).done(function(){
        	while_changing_load_track = false;
        	$grand_parent.removeClass("is-executing");
        });
	}
	...
	function init_track_page_load_time(){
		$toogle_el.track_load_time = $('input[name="track_page_load_time"]');
		if( $toogle_el.track_load_time.length ){
			$toogle_el.track_load_time.on('change', on_change_track_load_time);
		}
	}
