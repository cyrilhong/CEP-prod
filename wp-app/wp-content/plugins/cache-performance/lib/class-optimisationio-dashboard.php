...
class Optimisationio_Dashboard {
				...
				if ( ! is_wp_error( $result ) ) {
					...
					switch( $post_req['slug'] ){
						...
						case 'cache-performance':
							...
							$ret['plugin_settings_content'] = ob_get_contents();
							ob_end_clean();
							$ret['track_load_time_settings_content'] = self::track_load_time__settings(false);
							break;
						case 'wp-image-compression':
							ob_start();
	...
	private static function display_stats__cache_and_database_and_wp_disable(){
		...
		<div class="addon-stats">
			...
			<ul class="cache-and-database-list">
				<li><?php esc_html_e('Pages average load time', 'optimisationio'); ?><span><?php echo $active_addon && 1 === (int) get_option( 'Optimisationio_track_page_load_time' ) ? Optimisationio::average_pages_load_time() : '<i class="n_a">' . self::$str_i18n['n/a'] . '</i>'; ?></span></li>
			</ul>
			<ul class="cache-and-database-list">
	...
	public static function track_load_time__settings( $echo = true ){
		if( $echo ){
			Optimisationio_Dashboard::track_load_time_settings();
			return true;
		}
		else{
			ob_start();
	...
	public static function track_load_time_settings(){
		?>
		<div class="sidebar-cloudinary-api sidebar-track-load-time">
		    <div class="sidebar-content-wrap">
		        <div class="addon-settings-content auto-table-layout">
		            <div class="field">
		                <div class="field-left"><?php esc_attr_e('Track my page load time', 'optimisationio'); ?></div>
		                <div class="field-right">
		                    <?php Optimisationio_Dashboard::checkbox_component('track_page_load_time', 1 === (int) get_option( 'Optimisationio_track_page_load_time' ) ); ?>
		                    <span class="optimising-images-icon"></span>
		                </div>
		            </div>
		        </div>
		    </div>
		...
		<?php
		wp_nonce_field( 'optimisationio-track-load-time-nonce', 'optimisationio_track_load_time_nonce' );
	}
}
