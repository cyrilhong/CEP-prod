...
/**
 * Plugin Name: SmilePay_FamiPort
 * Plugin URI: http://www.smilepay.net
 * Description: SmilePay FamiPort
 * Author: SmilePay
...
add_action('plugins_loaded', 'SmilePayfamiport_gateway_init', 0);
function SmilePayfamiport_gateway_init() {
   if (!class_exists('WC_Payment_Gateway')) {
        return;
    }
    class WC_SmilePayfamiport extends WC_Payment_Gateway {
        public $title;
        public $description;
        public $dcvc;
        public $Rvg2c;
        ...
        public function __construct() {
            $this->id = 'smilepayfamiport';
            $this->icon = apply_filters('woocommerce_SmilePayfamiport_icon', plugins_url('log/famiport.jpg', __FILE__));
            $this->has_fields = false;
            $this->method_title = __('SmilePayfamiport', 'woocommerce');
            ...
            // Actions
            //add_action('init', array(&$this, 'check_SmilePayfamiport_response'));
        ...
        public function init_form_fields() {  //後台設置欄位
            ...
            $this->form_fields = array(
                'enabled' => array(
                    'title' => __(u2bf("啟用/關閉"), 'woocommerce'),
                    'type' => 'checkbox',
                    'label' => __(u2bf('SmilePay 全家FamiPort'), 'woocommerce'),
                    'default' => 'yes'
                ),
                'title' => array(
                    'title' => __(u2bf('標題'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('顧客在結帳時所顯示的付款方式標題'), 'woocommerce'),
                    'default' => __(u2bf('SmilePay 全家FamiPort 繳費'), 'woocommerce')
                ),
                'description' => array(
                    'title' => __(u2bf('付款方式說明'), 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __(u2bf('顧客在選擇付款方式時所顯示的介紹文字'), 'woocommerce'),
                    'default' => __(u2bf("SmilePay 全家FamiPort 繳費"), 'woocommerce')
                ),
                'dcvc' => array(
                    'title' => __(u2bf('商家代號'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('請填入您SmilePay商店代號'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Rvg2c' => array(
                    'title' => __(u2bf('商家參數碼'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('請填入您SmilePay商家參數碼'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Verify_key' => array(
                    'title' => __(u2bf('商家檢查碼'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('請填入您SmilePay商家檢查碼，檢查碼於商家後台「背景取號API」頁面中，請複製並貼入上方欄位'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Mid_smilepay' => array(
                    'title' => __(u2bf('商家驗證參數'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('請填入您SmilePa驗證碼，驗證碼於商家後台「基本資料管理」頁面中，請複製並貼入上方欄位，如不需驗證請保留空白'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),				
				'Deadline_date' => array(
                    'title' => __(u2bf('繳款截止期限'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('繳款截止期限，預設值為7日，請填入1~6，不得大於7日'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
				'Order_OKmain' => array(
                    'title' => __(u2bf('下單成功顯示訊息'), 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __(u2bf('下單成功顯示訊息'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
				/*'reurl' => array(
                    'title' => __(u2bf('交易完成回送位置'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bf('交易完成回送位置，請將下列文字複製到上方框內(如不須回送請留空白)</br><font color=red ue size=+1>'.$urll.'</font>', 'woocommerce')),
                    'default' => __('', 'woocommerce')
                )*/
            );
        }
         ...
         * @return void
         */
       public function get_SmilePayfamiport_args($order) {
            global $woocommerce;
            ...
            $SmilePayfamiport_args = apply_filters('woocommerce_SmilePayfamiport_args', $SmilePayfamiport_args);
            return $SmilePayfamiport_args;
        }
		
       public function thankyou_page($order_id) {  //接收回傳參數驗證  與   famiport取號
			...
			$SmilePayfamiport_args = $this->get_SmilePayfamiport_args($order);
			$order_status=$SmilePayfamiport_args['post_status'];
		if($order_status=='wc-pending')
		{
			...
			if(is_numeric($SmilePayfamiport_args['Deadline_date'])&&$SmilePayfamiport_args['Deadline_date']<6)
			{$Deadline_date_value=$SmilePayfamiport_args['Deadline_date'];}
			else
            ...
            if(is_ssl())
                $SmilePayfamiport_args['reurl']=str_replace('http://','https://',$SmilePayfamiport_args['reurl']);
            else
                $SmilePayfamiport_args['reurl']=str_replace('https://','http://',$SmilePayfamiport_args['reurl']);		
          	//取號
			if($SmilePayfamiport_args['reurl']!=''){$Vk=$SmilePayfamiport_args['reurl']."=".$order->get_order_key();}else{$Vk='';}
			...
			$post_str = 'Dcvc=' . $SmilePayfamiport_args['dcvc'] .
					'&Rvg2c=' . $SmilePayfamiport_args['Rvg2c'] .
					'&Pur_name=' .$pur_name.
					'&Tel_number=' .$order->get_billing_phone().
                    '&Mobile_number=' .$order->get_billing_phone().
					'&Address=' .$order->get_billing_address_1().
					'&Email=' .$order->get_billing_email().
					'&Data_id=' . $SmilePayfamiport_args['od_sob'].
					'&od_sob=' . $SmilePayfamiport_args['od_sob'].
					'&Amount=' . $SmilePayfamiport_args['amt'].
					'&Deadline_date=' . $Deadline_date.
					'&Roturl_status=' . 'woook'.
					'&Verify_key=' . $SmilePayfamiport_args['Verify_key'].
                    '&Remark=woocommerce';
					 //<SmilePay>item add start
					//'&Remark='. $products;
                     //<SmilePay>item add end
			$post_strall=$post_str.'&Pay_zg=6'.'&Roturl=' . $Vk;
			...
			{
				//寫入訂單繳費資料-消費者
                $title = b2uf($this->title);
				$ordertext="繳費方式：<font color=red>$title</font><br>繳費代碼：<font color=red>".$xml->FamiNO."</font><br>繳費期限：<font color=red>".$xml->PayEndDate."</font></br>如超過繳費期限請勿繳費</br>";
				$order->add_order_note(u2bf($ordertext),"1");	
				$ordertext="請勿刪除此資訊，速買配SmilePay追蹤碼：".$xml->SmilePayNO;
				$order->add_order_note(u2bf($ordertext),"0");
				//修改訂單狀態	
				$order->update_status( 'on-hold' );	
				//網頁顯示內容
				echo u2bf("<font color='red' size='3'>注意：訂單已成立，尚未付款，請依繳款資料盡速付款</font>");
				$showtext= "<br><br><h1>繳費資訊</h1>繳費方式：<font color='red' size='3'>$title</font></td></tr>";
				$showtext=$showtext."<br>繳費代碼：<font color='red' size='3'>".$xml->FamiNO."</font>";
				$showtext=$showtext."<br>繳費期限：<font color='red' size='3'>".$xml->PayEndDate."</font>";
				echo u2bf($showtext);
				echo u2bf("<br><font color='red' size='3'>如忘記<u>繳費資訊</u>請自上方選單列<u>帳號設定->訂單->檢視</u></font>");
				echo "<br><br>".$SmilePayfamiport_args['Order_OKmain'];
				//--物流start
				if($getc2cu=="Y")
				{
                    if( $b2c_run) //<b2c> start 
                    {
					    ...
					    $order->add_order_note(u2bf($scvurls),"0");
					    $scvurls_1="7-11取貨門市：<font color=red>".b2uf($b2c_storename.$b2c_storeid)."</font><br>";
                        $scvurls_1 .= "門市地址：<font color=red>".b2uf($b2c_storeaddress)."</font><br>";
                        $scvurls_1 .= "取貨人姓名：<font color=red>".b2uf($pur_name)."</font><br>";
                        $scvurls_1 .= "取貨人電話：<font color=red>".b2uf($order->get_billing_phone())."</font><br>";
                    ...
                    else //<b2c> end
                    {
                        ...
                        //<smilepay> modify address end			
					    $scvurls=repeatc2c($c2c_store,$post_str,$SmilePayfamiport_args['dcvc'],$SmilePayfamiport_args['Verify_key']);
					    $order->add_order_note(u2bf($scvurls),"0");
					    $scvurls_1="7-11取貨門市：<font color=red>".b2uf($c2c_storename.$c2c_storeid)."</font><br>";
                        $scvurls_1 .= "門市地址：<font color=red>".b2uf($c2c_storeaddress)."</font><br>";
                        $scvurls_1 .= "取貨人姓名：<font color=red>".b2uf($pur_name)."</font><br>";
                        $scvurls_1 .= "取貨人電話：<font color=red>".b2uf($order->get_billing_phone())."</font><br>";
                       
                    }
					$order->add_order_note(u2bf($scvurls_1),"1");
					echo u2bf('<h1>取貨資訊</h1>' . $scvurls_1);
				}
				//--物流end
			...
			else
			{
				...
				$order->add_order_note(u2bf($ordererror),"1");
				$order->update_status( 'cancelled' );
                echo u2bf($ordererror);					
			}
		}
		else
		{
			echo u2bf("<font color='red' size='3'>如忘記<u>繳費資訊</u>請自上方選單列<u>帳號設定->訂單->檢視</u></font>");
		}			
        ...
        public function process_payment($order_id) {
            ...
            {
                wc_add_notice(u2bf("<br>速買配商家參數錯誤，請與商家聯繫<br>") , 'error' );
                return;
            }
			/*//--物流start
			$shipping_method= $order->get_shipping_method();
            ...
            //if($shipping_method==u2bf("速買配7-11門市純取貨"))
			{
				...
				if(substr($order->billing_phone,0,2)!="09"||mb_strlen($order->billing_phone)!=10||mb_strlen( $username, "utf-8")>$limit_name_number){
                    wc_add_notice( __(u2bf("<br>電話格式錯誤，範例09XX123456(共10碼)。<br>消費者姓名不可超過5個字，純英文則不可超過10個字。請再次確認。<br>"), 'woothemes') . $error_message, 'error' );
                    return;
					exit;
				}
			}
            if($smilepayc2cp_obj && $smilepayc2cp_obj->isSelectedSmilepayC2Cp($order_id))
			//if($shipping_method==u2bf("速買配7-11門市取貨付款"))
			{
                wc_add_notice( __(u2bf("<br><font color='red'>配送方式選擇錯誤，不可選擇此配送方式</font><br>"), 'woothemes') . $error_message, 'error' );
                return;
                exit;
			}
            //<b2c> start
     ...
     * @return array
     */
    function add_SmilePayfamiport_gateway($methods) {
        $methods[] = 'WC_SmilePayfamiport';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_SmilePayfamiport_gateway');
}
function u2bf($text)//畫面輸出
{	return iconv("big5","UTF-8",$text);}
function b2uf($text)//寫入資料庫
