<?php
 ...
 $smseid = WC_Smilepayezcatup_Shipping_Method::get_order_smseid($order_id);
 $tracknum = WC_Smilepayezcatup_Shipping_Method::get_order_tracknum($order_id);
...
<?php if((!isset($smseid) || empty($smseid)) ||  (!isset($tracknum) || empty($tracknum))):?>
            <div>
            寄送包裹的長寬高(公分cm):<input type="text" name="sp_length" maxlength="6" style="width: 30px;" id="smilepayezcatup_length"  /> X
            <input type="text" name="sp_width" maxlength="6" style="width: 30px;" id="smilepayezcatup_width" /> X
            <input type="text" name="sp_height" maxlength="6" style="width: 30px;" id="smilepayezcatup_height" /><br>
            </div>

            <div>操作：
            
                    <button style="margin-left: 10px;" type="button" onclick="smilepayezcatup_create();" id="smilepayei_create_btn">產生取貨訂單</button>
...
<script type="text/javascript">
    ...
    function smilepayezcatup_create()
    {

        if(checkSmilepayEzcatupForm())
        {
            var $=jQuery;
            ...
            window.open('<?php echo admin_url('admin.php?page=smilepay_ezcatup_api&api_action=create_order&post=' . $_REQUEST['post']);  ?>'+arg,'_self');
        }
    ...
    function checkSmilepayEzcatupForm() {
            ...
            if (CheckDecimal(sp_length) && CheckDecimal(sp_width) && CheckDecimal(sp_height))
            {
                ...
                if(shippingcode == "smilepay_ezcatup_fridge" || shippingcode == "smilepay_ezcatup_freeze")
                {
                    if(lwh_sum > 120)
                    {
