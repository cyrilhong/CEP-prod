    ...
    class WC_SmilePayibon extends WC_Payment_Gateway {
		...
		//	$urll=get_option('siteurl')."/?respond";
            $this->form_fields = array(
                'enabled' => array(
                    'title' => __(u2bi("啟用/關閉"), 'woocommerce'),
                    'type' => 'checkbox',
                    'label' => __(u2bi("SmilePay 7-11 ibon"), 'woocommerce'),
                    'default' => 'yes'
                ),
                'title' => array(
                    'title' => __(u2bi('標題'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('顧客在結帳時所顯示的付款方式標題'), 'woocommerce'),
                    'default' => __(u2bi('SmilePay速買配 7-11 ibon 繳費'), 'woocommerce')
                ),
                'description' => array(
                    'title' => __(u2bi('付款方式說明'), 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __(u2bi('顧客在選擇付款方式時所顯示的介紹文字'), 'woocommerce'),
                    'default' => __(u2bi("SmilePay速買配 7-11 ibon 繳費"), 'woocommerce')
                ),
                'dcvc' => array(
                    'title' => __(u2bi('商家代號'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('請填入您SmilePay商店代號'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Rvg2c' => array(
                    'title' => __(u2bi('商家參數碼'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('請填入您SmilePay商家參數碼'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Verify_key' => array(
                    'title' => __(u2bi('商家檢查碼'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('請填入您SmilePay商家檢查碼，檢查碼於商家後台「背景取號API」頁面中，請複製並貼入上方欄位'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Mid_smilepay' => array(
                    'title' => __(u2bi('商家驗證參數'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('請填入您SmilePa驗證碼，驗證碼於商家後台「基本資料管理」頁面中，請複製並貼入上方欄位，如不需驗證請保留空白'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),				
                'Deadline_date' => array(
                    'title' => __(u2bi('繳款截止期限'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('繳款截止期限，預設值為7日，請填入1~6，不得大於7日'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
                'Order_OKmain' => array(
                    'title' => __(u2bi('下單成功顯示訊息'), 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __(u2bi('下單成功顯示訊息'), 'woocommerce'),
                    'default' => __('', 'woocommerce')
                ),
			/*	'reurl' => array(
                    'title' => __(u2bi('交易完成回送位置'), 'woocommerce'),
                    'type' => 'text',
                    'description' => __(u2bi('交易完成回送位置，請將下列文字複製到上方框內(如不須回送請留空白)</br><font color=red ue size=+1>'.$urll.'</font>', 'woocommerce')),
                    'default' => __('', 'woocommerce')
                )*/
            );
        }
       ...
       public function get_SmilePayibon_args($order) {
		...
		public function thankyou_page($order_id) {  //接收回傳參數驗證  與  7-11 Ibon取號
		...
		if($order_status=='wc-pending')
		{
			...
			if(strpos($string,"<SmilePay>")&&$Status=="1")
			{
				...
				$ordertext="繳費方式：<font color=red>$title</font><br>繳費代碼：<font color=red>".$xml->IbonNo."</font><br>繳費期限：<font color=red>".$xml->PayEndDate."</font></br>如超過繳費期限請勿繳費</br>";
				...
				//網頁顯示內容
				echo u2bi("<font color='red' size='3'>注意：訂單已成立，尚未付款，請依繳款資料盡速付款</font>");
				$showtext= "<br><br><h1>繳費資訊</h1>繳費方式：<font color='red' size='3'>$title</font></td></tr>";
				$showtext=$showtext."<br>繳費代碼：<font color='red' size='3'>".$xml->IbonNo."</font>";
				$showtext=$showtext."<br>繳費期限：<font color='red' size='3'>".$xml->PayEndDate."</font>";
				echo u2bi($showtext);
				echo u2bi("<br><font color='red' size='3'>如忘記<u>繳費資訊</u>請自上方選單列<u>帳號設定->訂單->檢視</u></font>");
				...
				if($getc2cu=="Y")
				{
                    if( $b2c_run) //<b2c> start 
                    {
					    ...
					    $scvurls_1="7-11取貨門市：<font color=red>".b2ui($b2c_storename.$b2c_storeid)."</font><br>";
                        $scvurls_1 .= "門市地址：<font color=red>".b2ui($b2c_storeaddress)."</font><br>";
                        $scvurls_1 .= "取貨人姓名：<font color=red>".b2ui($pur_name)."</font><br>";
                        $scvurls_1 .= "取貨人電話：<font color=red>".b2ui($order->get_billing_phone())."</font><br>";
                    ...
                    else //<b2c> end
                    {
					    ...
					    $order->add_order_note(u2bi($scvurls),"0");
					    $scvurls_1="7-11取貨門市：<font color=red>".b2ui($c2c_storename.$c2c_storeid)."</font><br>";
                        $scvurls_1 .= "門市地址：<font color=red>".b2ui($c2c_storeaddress)."</font><br>";
                        $scvurls_1 .= "取貨人姓名：<font color=red>".b2ui($pur_name)."</font><br>";
                        $scvurls_1 .= "取貨人電話：<font color=red>".b2ui($order->get_billing_phone())."</font><br>";
                       
                    }
					$order->add_order_note(u2bi($scvurls_1),"1");
                    echo u2bi('<h1>取貨資訊</h1>' . $scvurls_1);
				}
				//--物流end
			}
			else
		...
		{
			echo u2bi("<font color='red' size='3'>如忘記<u>繳費資訊</u>請自上方選單列<u>帳號設定->訂單->檢視</u></font>");
		}			
		...
		//添加判斷是否可用
            ...
            {
                wc_add_notice(u2bi("<br>速買配商家參數錯誤，請與商家聯繫<br>") , 'error' );
                return;
            }
            ...
            //if($shipping_method==u2bi("速買配7-11門市純取貨"))
			{
				...
				if(substr($order->billing_phone,0,2)!="09"||mb_strlen($order->billing_phone)!=10||mb_strlen( $username, "utf-8")>$limit_name_number){
                    wc_add_notice( __(u2bi("<br>電話格式錯誤，範例09XX123456(共10碼)。<br>消費者姓名不可超過5個字，純英文則不可超過10個字。請再次確認。<br>"), 'woothemes') . $error_message, 'error' );
                    return;					
					exit;
				}
			}
            if($smilepayc2cp_obj && $smilepayc2cp_obj->isSelectedSmilepayC2Cp($order_id))
			//if($shipping_method==u2bi("速買配7-11門市取貨付款"))
			{
                wc_add_notice( __(u2bi("<br><font color='red'>配送方式選擇錯誤，不可選擇此配送方式</font><br>"), 'woothemes') . $error_message, 'error' );
                return;
                exit;
			}
            //<b2c> start
...
function u2bi($text)//畫面輸出
{	return iconv("big5","UTF-8",$text);}
function b2ui($text)//寫入資料庫
